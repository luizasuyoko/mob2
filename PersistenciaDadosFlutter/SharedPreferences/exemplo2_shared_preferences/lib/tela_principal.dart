...
class _TelaPrincipalState extends State<TelaPrincipal>{
  ...
  _adicionarTarefa() async{
    ...
    if(_tarefaController.text.trim().isEmpty){
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("A tarefa esta vazia"),));
    }else{
      _tarefas.add(_tarefaController.text.trim());
      prefs.setStringList(_nome, _tarefas);
      setState(() {
        _tarefaController.clear();
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("A tarefa adicionada com sucesso"),));
        Navigator.pop(context); //fecho o alertdialog
      });
    }
  }
}